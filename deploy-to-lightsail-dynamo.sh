#!/bin/bash
# Deployment script for AWS Lightsail Containers with DynamoDB
...
# 3. Created the necessary secrets and DynamoDB tables with setup-aws-dynamodb.sh
# 4. Created the storage bucket with setup-storage-bucket.sh

# Check for AWS CLI
if ! command -v aws &> /dev/null; then
    echo "AWS CLI not found. Please install it first with ./install-awscli.sh"
    exit 1
fi
...
# Create a deployment with DynamoDB configuration
echo "Creating deployment..."
aws lightsail create-container-service-deployment \
    --service-name "$SERVICE_NAME" \
    --region "$REGION" \
    --containers "{\"app\":{\"image\":\"$IMAGE_TAG\",\"ports\":{\"10000\":\"HTTP\"},\"environment\":{\"FLASK_ENV\":\"production\",\"PORT\":\"10000\",\"HUGGINGFACE_API_KEY\":\"{{ssm:HUGGINGFACE_API_KEY}}\",\"SECRET_KEY\":\"{{ssm:SECRET_KEY}}\",\"JWT_SECRET\":\"{{ssm:JWT_SECRET}}\",\"USE_DYNAMODB\":\"true\",\"DYNAMODB_REGION\":\"$REGION\"}}}" \
