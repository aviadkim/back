...
class AnalyticsAgent:
    ...
    def analyze_portfolio_trends(self, user_id: str, time_period: int = 180) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'security_type' in df.columns and 'portfolio_weight' in df.columns:
                ...
                latest_data = [row for table in latest_doc.financial_data.values()
                              for row in table.get('dataframe', [])]
                
                if latest_data:
                    latest_df = pd.DataFrame(latest_data)
                    if 'security_type' in latest_df.columns and 'portfolio_weight' in latest_df.columns:
                        composition = latest_df.groupby('security_type')['portfolio_weight'].sum()
    ...
    def detect_outliers(self, user_id: str, metric: str = 'yield_percent',
                        z_score_threshold: float = 2.0) -> Dict[str, Any]:
        ...
        try:
                            ...
                            for row in table_info['dataframe']:
                                if metric in row:
                                    financial_data.append({
                                        ...
                                        'value': row[metric],
                                        'security_name': row.get('security_name', 'Unknown'),
            ...
            result = {
                ...
                "outliers_percentage": (len(outliers) / len(df)) * 100 if len(df) > 0 else 0,
                "outliers": []
            }
    ...
    def _calculate_trend(self, df: pd.DataFrame, date_col: str, 
                         value_col: str) -> float:
