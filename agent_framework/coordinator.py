    ...
    def _setup_llm(self):
        ...
        if self.llm_provider == "huggingface" and self.huggingface_api_key:
            ...
            try:
                ...
                logger.info(
                    "Using Hugging Face model for agent (as configured in LLM_PROVIDER)."
                )
            except ImportError:
                logger.warning(
                    "Hugging Face Hub package not installed. Falling back to other providers."
                )
                self.llm = None
            ...
            try:
                ...
                logger.info(
                    "Using Mistral AI for agent (as configured in LLM_PROVIDER)."
                )
            except ImportError:
                logger.warning(
                    "Mistral AI package not installed. Falling back to other providers."
                )
                self.llm = None
        elif self.llm_provider == "openai" and self.openai_api_key:
            # Use OpenAI if specifically configured
            self.llm = ChatOpenAI(
                model_name="gpt-4o", temperature=0, api_key=self.openai_api_key
            )
            logger.info(
                "Using OpenAI GPT-4o for agent (as configured in LLM_PROVIDER)."
            )
        else:
            ...
            if self.huggingface_api_key:
                ...
                # No LLM available
                logger.warning(
                    "No API keys found for any LLM provider. AI functionality will be limited."
                )
                self.llm = None
                self.llm_provider = None
        ...
        try:
            ...
            if self.llm_provider == "openai":
                with get_openai_callback() as cb:
                    ...
                    logger.info(
                        f"OpenAI Analysis completed with {cb.total_tokens} tokens ({cb.prompt_tokens} prompt, {cb.completion_tokens} completion)"
                    )
            else:
            ...
            try:
                ...
                logger.error(
                    f"Failed to parse analysis result as JSON: {result[:500]}..."
                )
    ...
    def process_query(
        ...
        try:
            ...
            truncated_context = self._truncate_text(
                combined_context, max_context_tokens
            )
            ...
            if self.llm_provider == "openai":
                with get_openai_callback() as cb:
                    ...
                    logger.info(
                        f"OpenAI Query processed with {cb.total_tokens} tokens ({cb.prompt_tokens} prompt, {cb.completion_tokens} completion)"
                    )
            else:
                # Run without OpenAI callback for other providers
                result = chain.run(
            ...
            return {
                "answer": "I'm sorry, but I encountered an error while processing your query. Please try again.",   # Removed f-string prefix
                "document_references": [],
                "suggested_questions": [],
            }
...
Be precise and ensure all data is correctly extracted. If you cannot find data for a requested column, use null or appropriate placeholders."""
            ...
            if self.llm_provider == "openai":
                with get_openai_callback() as cb:
                    result = chain.run(context=truncated_context, spec=formatted_spec)
                    logger.info(
                        f"OpenAI Table generation completed with {cb.total_tokens} tokens ({cb.prompt_tokens} prompt, {cb.completion_tokens} completion)"
                    )
            else:
                # Run without OpenAI callback for other providers
                result = chain.run(context=truncated_context, spec=formatted_spec)
                ...
                logger.info(
                    f"Table generation completed using {self.llm_provider} provider."
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to parse table generation result as JSON: {result[:500]}..."
                )
...
4. Written in the same language as the conversation"""
                ...
                try:
                    ...
                    # If JSON parsing fails, log it and fall through to the fallback
                    logger.warning(
                        f"Failed to parse LLM response for suggested questions as JSON: {result[:100]}..."
                    )
